# tiny-model：在本地训练你的中文小说续写模型

在 **8G 显存**（如 RTX 4060 Ti）上，从零训练一个 **7M～70M 参数**的 GPT，学会续写你投喂的那类小说。

不需要云端 GPU，不需要 HuggingFace 账号，**上传 txt → 点几下 → 看曲线 → 试续写**，全程可在浏览器里完成。
...
| **省显存设计** | bf16、梯度累积、memmap 读数据，8G 卡可跑 |
...
| 预设 | 参数量 | 推荐语料量 | 层 / 头 / 维度 |
|------|--------|------------|----------------|
| `micro` | ~7M | < 5 MB | 4 / 4 / 256 |
| `tiny` | ~16M | 5～30 MB | 6 / 6 / 384 |
| `small` | ~33M | 30～150 MB | 8 / 8 / 512 |
| `medium` | ~70M | > 150 MB | 12 / 10 / 640 |
